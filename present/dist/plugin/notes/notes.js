	...
	<body>
		...
		<script>
			...
			(function() {
				...
				/**
				 * Send navigation to the authoritative main deck and mirror the
				 * same command locally so the speaker previews update immediately.
				 */
				function dispatchNavigationCommand( method, args = [], updatePreviews = true ) {

					window.opener.postMessage( JSON.stringify({ method: method, args: args }), '*' );
					if( !updatePreviews ) return;

					const currentReveal = currentSlide && currentSlide.contentWindow.Reveal;
					const upcomingReveal = upcomingSlide && upcomingSlide.contentWindow.Reveal;
					if( !currentReveal || !currentReveal.isReady() || typeof currentReveal[method] !== 'function' ) return;

					currentReveal[method].apply( currentReveal, args );
					if( upcomingReveal && upcomingReveal.isReady() ) {
						const { previewVideo, previewImage, previewIframe, ...currentPreviewState } = currentReveal.getState();
						upcomingReveal.setState( currentPreviewState );
						upcomingReveal.next();
					}
				...
				/**
				 * Forward keyboard events directly to the main presentation.
				 * Keeping the main window authoritative avoids races with
				 * state updates from the current-slide preview.
				 *
				...
				function setupKeyboard() {
					...
					document.addEventListener( 'keydown', function( event ) {
						...
						dispatchNavigationCommand( 'triggerKey', [ event.keyCode ], navigationKeys.includes( event.keyCode ) );
					} );
