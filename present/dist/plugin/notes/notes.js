	...
	<head>
		...
		<style>
			...
			#speaker-controls {
				...
				.speaker-controls-notes .value video,
				.speaker-controls-notes .value svg {
					max-width: 100%;
					max-height: 35%;
					object-fit: contain;
				}

				.speaker-controls-notes .value pre {
		...
		<script>
			...
			(function() {
				...
				var notes,
					...
					connected = false,
					lastStateApplyTime = 0,
					pendingStateMessage = null,
					stateApplyTimer = null,
					appliedNotesKey = null
				...
				window.addEventListener( 'message', function( event ) {
					...
					// Ignore same-origin messages that aren't ours, e.g. from
				 ...
				 *
				 * States are applied latest-wins: rapid sequences (e.g. a
				 * held-down navigation key) are coalesced so that rendering
				 * never backlogs and the final state is always applied.
				 * A lone message is applied immediately; during bursts we
				 * keep only the most recent state and apply it from a short
				 * trailing timeout.
				 */
				function handleStateMessage( data ) {
					...
					if( now - lastStateApplyTime > 33 && !stateApplyTimer ) {
						lastStateApplyTime = now;
						applyStateMessage( data );
					}
					else {
						pendingStateMessage = data;
						if( !stateApplyTimer ) {
							stateApplyTimer = setTimeout( function() {
								stateApplyTimer = null;
								lastStateApplyTime = Date.now();
								if( pendingStateMessage ) {
									var pending = pendingStateMessage;
									pendingStateMessage = null;
									applyStateMessage( pending );
								}
							}, 33 );
						}
					}
				...
				/**
				 * Renders a state message from the main window. The main
				 * window is the single source of truth; the notes and both
				 * preview frames are updated together from the same message
				 * so they can never disagree with each other.
				 */
				function applyStateMessage( data ) {

					if ( data.notes ) {
						// Skip re-rendering when the notes are unchanged, e.g.
						// when stepping through fragments of the same slide
						var notesKey = ( data.markdown ? 'md:' : 'html:' ) + data.whitespace + ':' + data.notes;
						if( notesKey !== appliedNotesKey ) {
							appliedNotesKey = notesKey;
				...
				/**
				 * Treat preview controls as commands for the main presentation.
				 * The preview must never navigate independently or publish state.
				 */
				function setupCurrentSlideNavigation() {

					const previewDocument = currentSlide.contentDocument;
					if( !previewDocument ) return;
					let lastTouchNavigation = 0;

					const handleControlNavigation = function( event ) {
						const button = event.target && typeof event.target.closest === 'function'
							? event.target.closest( '.controls button' )
							: null;
						if( !button ) return;
						...
						const navigation = navigationMethods.find( ([ className ]) => button.classList.contains( className ) );
						...
						else if( event.type === 'touchend' || Date.now() - lastTouchNavigation < 750 ) {
							return;
						}

						dispatchNavigationCommand( navigation[1] );
					};

					[ 'touchstart', 'touchend', 'click' ].forEach( eventName => {
						previewDocument.addEventListener( eventName, handleControlNavigation, { capture: true, passive: false } );
					} );

				}

				/**
				 * Send navigation to the authoritative main deck. The previews
				 * are never navigated directly; they only follow the state
				 * messages that the main deck posts back, so a command can
				 * never leave the speaker view and the presentation showing
				 * different slides.
				 */
				function dispatchNavigationCommand( method, args = [] ) {

					window.opener.postMessage( JSON.stringify({ method: method, args: args }), '*' );

				}

				/**
				 * Forward keyboard events to the main presentation window.
				 * This handler serves the speaker view document and both
				 * preview iframe documents, so navigation works no matter
				 * which part of the speaker view has focus.
				 *
				 * Block F5 default handling, it reloads and disconnects
				 * the speaker notes window.
				 */
				function onSpeakerKeydown( event ) {
					...
					// Don't steal keys from form controls, e.g. the layout dropdown
					if( event.target && event.target.tagName && /select|input|textarea/i.test( event.target.tagName ) ) {
						return;
					}

					const navigationKeys = [ 32, 33, 34, 35, 36, 37, 38, 39, 40, 72, 74, 75, 76, 78, 80 ];
					if( navigationKeys.includes( event.keyCode ) ) {
						event.preventDefault();
					}

					dispatchNavigationCommand( 'triggerKey', [ event.keyCode ] );

				}

				function setupKeyboard() {

					document.addEventListener( 'keydown', onSpeakerKeydown );

				}

				/**
				 * Keys pressed while a preview iframe has focus never reach
				 * this document, so relay them from inside the frame. The
				 * previews are loaded with keyboard=false and can therefore
				 * never navigate themselves.
				 */
				function attachPreviewKeyboardRelay( frame ) {

					frame.addEventListener( 'load', function() {
						try {
							frame.contentDocument.addEventListener( 'keydown', onSpeakerKeydown );
						}
						catch ( error ) { console.warn( error ) }
					} );
				...
				function setupIframes( data ) {
					...
					var upcomingURL = data.url + urlSeparator + params + '&scrollActivationWidth=false&controls=false&speakerPreview=upcoming&postMessageEvents=true' + hash;

					currentSlide = document.createElement( 'iframe' );
					currentSlide.addEventListener( 'load', setupCurrentSlideNavigation );
					attachPreviewKeyboardRelay( currentSlide );
					...
					attachPreviewKeyboardRelay( upcomingSlide );
				...
				function setupNotes() {
					...
					window.addEventListener( 'resize', scheduleFitSpeakerNotes );
					if( document.fonts && document.fonts.ready ) document.fonts.ready.then( scheduleFitSpeakerNotes );
					if( typeof ResizeObserver === 'function' ) {
						new ResizeObserver( scheduleFitSpeakerNotes ).observe( document.querySelector( '#speaker-controls' ) );
					}

				}

				/**
				 * Fit all notes into their fixed pane at the largest readable size.
				 * The pane never scrolls; its font size adapts to the available area.
				 */
				function fitSpeakerNotes() {

					if( !notes || notes.classList.contains( 'hidden' ) || !notesValue.clientHeight ) return;

					let smallest = 1;
					let largest = 48;
					let best = smallest;
					const fits = () => notesValue.scrollHeight <= notesValue.clientHeight + 1 &&
						notesValue.scrollWidth <= notesValue.clientWidth + 1;

					while( largest - smallest > 0.25 ) {
						const candidate = ( smallest + largest ) / 2;
						notesValue.style.fontSize = candidate + 'px';
						if( fits() ) {
							best = candidate;
							smallest = candidate;
						}
					...
					notesValue.style.fontSize = best + 'px';
					notesValue.querySelectorAll( 'img' ).forEach( image => {
						if( !image.complete ) image.addEventListener( 'load', scheduleFitSpeakerNotes, { once: true } );
					} );

				}

				function scheduleFitSpeakerNotes() {

					cancelAnimationFrame( fitNotesFrame );
					fitNotesFrame = requestAnimationFrame( fitSpeakerNotes );
